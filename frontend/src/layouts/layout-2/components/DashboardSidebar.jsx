...
// CUSTOM DEFINED HOOK
import useLayout from '../../layout-2/context/useLayout';
...
export default function LayoutSideBar() {
  ...
  // ---- Filtro por rol para navigation (iconos principales) ----
  const filterNavigationByRole = useCallback(
    (items) => {
      if (isAdmin) return items; // admin ve todo
        ...
        .map((item) => {
          ...
          if (item.children) {
            const filteredChildren = item.children.filter(
              (child) =>
                !child.roles || !roleId || child.roles.includes(roleId)
            );
  ...
  // ---- Filtro por rol para el submenú (por seguridad extra) ----
  const filteredCategoryMenus = useMemo(() => {
    if (isAdmin) return categoryMenus;
    if (!roleId) return [];

    return categoryMenus.filter(
      (item) => !item.roles || item.roles.includes(roleId)
    );
  }, [categoryMenus, roleId, isAdmin]);
    ...
    <Fragment>
      ...
      >
        <div className="navigation-list">
          {filteredNavigation.map((nav, index) => (
  ...
  const SECONDARY_SIDEBAR_CONTENT = (
    <Fragment>
      ...
      >
        {filteredCategoryMenus.map(({ name, path }) => (
          <SubMenuItem
            key={name}
